...
<Tabs items={["Next.js", "JavaScript", "Vue"]}>
  ...
  <Tab>
    To add custom pages to the `<OrganizationProfile />` component using the <SDKLink href="/docs/reference/javascript/overview" sdks={["js-frontend"]}>JavaScript SDK</SDKLink>, pass the `customPages` property to the `mountOrganizationProfile()` or `openOrganizationProfile()` method, as shown in the following example:
