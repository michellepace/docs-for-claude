...
lastUpdated: 2025-12-02T19:13:47.000Z
sdkScoped: "false"
...
To avoid sending an email or SMS message with a one-time password (<Tooltip><TooltipTrigger>OTP</TooltipTrigger><TooltipContent>An **OTP** stands for one-time password and is a code that is used to authenticate a user. The OTP is typically sent to a user's email address or phone number and must be entered within a certain time period to be valid.</TooltipContent></Tooltip>) during testing, you can use a fake email address or phone number that has a fixed code. Read the complete documentation [here](/docs/guides/development/testing/test-emails-and-phones).
